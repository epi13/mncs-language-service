...
This repository may cache, index, project, explain, route, and bind semantic information to workspace snapshots. It must not silently reinterpret language meaning. Language reference facts are consumed from the content-addressed capability index generated by `mncs-language`; the service adds query projections, not a second reference.
