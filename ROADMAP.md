...
## Phase 4.6 — First MNCS-native semantic query kernel

**Status: implemented / experimental.**

The service now executes one real consumer-level query in MNCS while retaining
the Rust implementation as a differential control:

- `mncs/status_query.mncs` imports the authoritative
  `mncs.core.status.v1::summarize8` lattice and exposes only a thin service
  adapter signature;
- `mncs-service-core::native_obligations` projects the exact bounded
  `PASS`/`FAIL`/`UNKNOWN` statuses generated by `mncs-language`, executes a
  frozen `mncs-research-bytecode` artifact, and validates its nominal
  `StatusSummary` identity and fields;
- the result retains source, dependency, and artifact identities and is
  `unsupported` when preparation, execution, validation, or differential
  agreement cannot be established;
- the compiled query artifact is reused for an unchanged query/dependency
  identity pair; the projection is bounded to eight obligations;
- the read-only MCP tool `native_obligations` exposes the path without moving
  filesystem, workspace, locks, LSP, MCP, or process responsibilities into
  MNCS.

This tranche exposed and fixed a general `mncs-language` module-linking issue:
imported bounded sequence signatures now preserve nominal element identities.
The upstream positive and fail-closed negative compiler tests, plus the service
